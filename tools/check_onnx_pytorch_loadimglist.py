...
def pytorch_onnx_test():
    ...
    for path in filename:
        # img_path = 'data/shengxiancheng-inside/leftImg8bit/val/shengxiancheng/2e119ca3-a0b5-494e-b1b5-d58b3b67ce8c_0_leftImg8bit.png'
        w, h = (128, 96)
        print(path)
        img = cv2.imread(os.path.join(srcPath, path))[:, :, ::-1]  # bgr->rgb
        resized = cv2.resize(img, (w, h))
        # cv2.imwrite("img_320_320.jpg", resized)
        ...
        onnx_out = np.array(sess.run([label_name], {input_name: to_numpy(tensor)})[0])  #fc 输出是三维列表
        # print("==============>")
        # print(onnx_out)
        # print(onnx_out.shape)
        # print("==============>")
        torch_out_res = pytorch_out(tensor).detach().numpy()   #fc输出是二维 列表
        # print(torch_out_res)
