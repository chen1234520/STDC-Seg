...
def parse_args():
    ...
    parse.add_argument(
            '--n_img_per_gpu',  # maybe is batchsize
            dest = 'n_img_per_gpu',
            type = int,
            default = 8,
            )
...
def train():
    ...
    dspth = '/root/chenguang/data/shengxiancheng-inside'
    ...
    torch.cuda.set_device(args.local_rank)  #指定显卡编号
    dist.init_process_group(
                backend = 'nccl',
                init_method = 'tcp://127.0.0.2:33280',
    ...
    ds = CityScapesShengXianCheng(dspth, cropsize=cropsize, mode=mode, randomscale=randomscale)
    ...
    ## model
    ignore_idx = 255
    net = BiSeNet_delNet(backbone=args.backbone, n_classes=n_classes, pretrain_model=args.pretrain_path, 
    use_boundary_2=use_boundary_2, use_boundary_4=use_boundary_4, use_boundary_8=use_boundary_8, 
    use_boundary_16=use_boundary_16, use_conv_last=args.use_conv_last)

    if not args.ckpt is None:
        net.load_state_dict(torch.load(args.ckpt, map_location='cpu'),strict = False)
        # net.load_state_dict(torch.load("/root/chenguang/project/STDC-Seg-master/checkpoints/STDC1-Seg-shiliu-96*96-20220106/pths/model_iter30000_mIOU50_0.4093_mIOU75_0.7057.pth", map_location='cpu'))
    ...
    net.cuda()
    net.train()
    net = nn.parallel.DistributedDataParallel(net,      #分布式训练
    ...
    for it in range(max_iter):
        ...
        #          out16   out16   out32    detail8_sp
        
        loss.backward()

        # 设置学习率调整方式
        # optim.step()    # Adam
        optim.step_step()   # step
